...
qa-gate.sh update [--ci]          bring the repo up to date with the installed gate: pin gateVersion, sync the
                                  history .gitignore rule, refresh .github/workflows/qa-gate.yml and the DoD block
                                  in AGENTS.md/CLAUDE.md; --ci creates the workflow when there is none
...
Exit codes: `0` PASS · `1` FAIL · `3` usage or internal error. `deploy` verifies a site after it was deployed (smoke +
compliance in live mode); it never deploys — that stays a human order run on the server.
...
| staging | `nuclei` | yes | Docker Nuclei, `web.nuclei.templates` at `severity`; the container reaches the host app through `host.docker.internal` → `qa-report/nuclei.jsonl`. A run that does not complete (unreachable target, bad invocation) is FAIL, not a clean site: the verdict records nuclei's exit code |
| deploy | `smoke` | yes | `--base-url` or `deploy.liveUrl`: https, `web.readyPath` answers 2xx/3xx within `deploy.smokeTimeoutSec`, no redirect to http; WARN above 3 s → `qa-report/smoke.json`. Then the compliance checks run against the live site and the run lands in the history as stage `deploy` |
...
| `deploy.liveUrl` / `smokeTimeoutSec` | `""` / 20 | the live site the `deploy` stage verifies (or pass `--base-url`) |
...
| `ai.promptGlobs` / `evalFile` / `ratchetFile` | prompt globs / `qa-report/ai-eval-latest.json` / `qa-report/ai-eval-ratchet.json` | where the AI evidence lives and which files make it stale. Distinct from `legal.ai`, which is what the *website* tells its visitors (KI-VO Art. 50). Commit both files: `init` adds the `.gitignore` exceptions |
