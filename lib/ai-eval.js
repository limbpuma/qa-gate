...
const DEFAULTS = {
  ...
  manifestFile: 'qa-report/ai-eval-manifest.json',
...
function verdict(repo, configPath) {
  ...
  // The manifest pins each case's category on the base branch (the file is on the gate-config guard list).
  // Without it the one non-waivable check can be dodged by re-tagging a safety case as quality — the artifact
  // was verified for existence and freshness, never for an honest taxonomy (v0.13 design review).
  const normCategory = (c) => (CATEGORIES.includes(c.category) ? c.category : 'quality');
  const manifest = (() => {
    const m = readJson(path.join(repo, cfg.manifestFile));
    if (!m || !Array.isArray(m.cases)) return { found: false, file: cfg.manifestFile, retagged: [], missingBlocking: [], unregistered: [] };
    const evidenceById = new Map(report.cases.filter((c) => c && typeof c.id === 'string').map((c) => [c.id, normCategory(c)]));
    const manifestById = new Map(m.cases.filter((c) => c && typeof c.id === 'string').map((c) => [c.id, normCategory(c)]));
    const retagged = [...manifestById].filter(([id, cat]) => evidenceById.has(id) && evidenceById.get(id) !== cat)
      .map(([id, cat]) => `${id}(${cat}->${evidenceById.get(id)})`);
    const missingBlocking = [...manifestById].filter(([id, cat]) => BLOCKING_CATEGORIES.includes(cat) && !evidenceById.has(id)).map(([id]) => id);
    const unregistered = [...evidenceById.keys()].filter((id) => !manifestById.has(id));
    return { found: true, file: cfg.manifestFile, retagged, missingBlocking, unregistered };
  })();
...
// The manifest is the committed truth about which case is which: id + category, nothing else. Changing it is
// a base-branch decision the gate-config guard makes visible, exactly like a waiver.
function manifestWrite(repo, configPath) {
  const config = readJson(configPath) || {};
  const cfg = settings(config);
  const report = readJson(path.join(repo, cfg.evalFile));
  if (!report || !Array.isArray(report.cases)) { process.stderr.write(`no evidence at ${cfg.evalFile}\n`); process.exit(1); }
  const norm = (c) => (CATEGORIES.includes(c.category) ? c.category : 'quality');
  const cases = report.cases.filter((c) => c && typeof c.id === 'string')
    .map((c) => ({ id: c.id, category: norm(c) }))
    .sort((a, b) => a.id.localeCompare(b.id));
  const out = path.join(repo, cfg.manifestFile);
  fs.mkdirSync(path.dirname(out), { recursive: true });
  fs.writeFileSync(out, JSON.stringify({ cases, at: new Date().toISOString() }, null, 2) + '\n');
  process.stdout.write(cfg.manifestFile + '\n');
}
...
function main() {
  ...
  if (cmd === 'manifest-write') return manifestWrite(repo, configPath);
