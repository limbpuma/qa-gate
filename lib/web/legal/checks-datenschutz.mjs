...
const REQUIRED_SECTIONS = [
  ...
  // Why the wider pattern: Du-form sites ("deiner Einwilligung") and "jederzeit mit Wirkung für die Zukunft widerrufen" are common wordings.
  { id: 'widerruf-einwilligung', regex: 'Widerruf(srecht)? (der|Ihrer|deiner|meiner) Einwilligung|jederzeit(\\s+mit\\s+Wirkung\\s+f(ü|ue)r\\s+die\\s+Zukunft)?\\s+widerrufen|jederzeit widerrufbar|withdraw', law: 'DSGVO Art. 13 Abs. 2 lit. c' },
...
// Consent is "in use" when the site shows a banner, loads third parties after accepting, or declares it required.
function consentInUse(s) {
  return Boolean(s.bannerSeen) || s.externalHosts(s.requestsAfter || []).length > 0 || Boolean(s.legal.consent && s.legal.consent.required);
}
...
export async function datenschutzContent(s, r) {
  ...
  // Why: Art. 13 Abs. 2 lit. c only applies where processing is based on consent; a landing page without a banner
  // and without third parties owes no withdrawal section.
  const required = consentInUse(s) ? REQUIRED_SECTIONS : REQUIRED_SECTIONS.filter((sec) => sec.id !== 'widerruf-einwilligung');
  const missing = required.filter((sec) => !new RegExp(sec.regex, 'i').test(text));
  if (!missing.length) return check(r.id, 'PASS', r.law, `all ${required.length} applicable Art. 13 sections present${required.length < REQUIRED_SECTIONS.length ? ' (no consent in use: withdrawal section not required)' : ''}`);
