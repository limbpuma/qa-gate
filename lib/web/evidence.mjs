...
function axeSection(axe) {
  ...
  const lines = [`- Tags: ${axe.tags.join(', ')} (warnings only: ${axe.warnTags.join(', ')})`,
    `- Blocking violations: **${axe.totals.blocking}** · warnings: ${axe.totals.warnings} · pages: ${axe.pages.length}`];
...
function complianceSection(c) {
  ...
  return ['| Check | Status | Law | Detail |', '|---|---|---|---|',
...
## 2. Accessibility — Pa11y (axe + HTML CodeSniffer, WCAG 2.1 AA)
...
pass, focus order, error identification in forms, content on zoom 200 %.
