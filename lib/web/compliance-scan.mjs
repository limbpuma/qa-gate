...
// --spec: the resolved business block (lib/spec.js output) for legal.spec-consistency.
// Shadow pass: rules of a feature the pages show signals for but the config does not declare run afterwards with
// FAIL → WARN 'shadow' and PASS → SKIP, so the cost of the drift is visible without changing the verdict.
...
// Rules skipped only because their feature is not declared, when the pages show that feature: run them anyway.
async function shadowPass(session, registry, checks, profile, declaredFeatures, today) {
  const seen = await core.seenFeatures(session);
  const undeclared = [...seen].filter((f) => !declaredFeatures.includes(f));
  if (!undeclared.length) return;
  const widened = [...declaredFeatures, ...undeclared];
  for (const rule of registry.rules) {
    const missing = (rule.requires || []).filter((f) => !declaredFeatures.includes(f));
    if (!missing.length || !missing.every((f) => undeclared.includes(f))) continue;
    if (skipReason(rule, profile, widened, today)) continue;
    ...
    const idx = checks.findIndex((c) => c.id === rule.id);
    const why = `shadow (feature ${missing.join(', ')} visible but not declared in legal.features)`;
    const shadow = result.status === 'PASS'
      ? check(rule.id, 'SKIP', rule.law, `${why}: would pass — ${result.detail}`)
      : check(rule.id, 'WARN', rule.law, `${why}: would ${result.status} — ${result.detail}`);
    shadow.shadow = true;
    if (idx >= 0) checks[idx] = shadow; else checks.push(shadow);
  }
}
...
async function main() {
  ...
  const spec = parseJson(opts.spec, null);
  ...
  try {
    ...
    if (!onlyRule) await shadowPass(session, registry, checks, profile, features, today);
  } catch (err) {
