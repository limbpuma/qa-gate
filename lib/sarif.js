...
function fromVerdictChecks(run, verdict) {
  const SELF_DESCRIBED = new Set(['semgrep', 'trivy-fs', 'trivy-image', 'legal', 'axe', 'secrets', 'audit', 'ai-eval-safety', 'ai-eval-quality']);
...
function fromAudit(run, report) {
  const ids = new Set();
  for (const f of (report && report.findings) || []) {
    const id = `audit.${f.package}`;
    run.rule(id, `${f.package} ${f.range}: ${f.title || 'vulnerable dependency'}`, f.url);
    run.add(id, f.severity === 'critical' ? 'error' : 'warning', `${f.package} ${f.range} (${f.severity}${f.direct ? ', direct' : ', transitive'}) — fix: ${f.fixAvailable}`, location('package.json'), { advisories: f.ids });
    for (const a of f.ids || []) ids.add(a);
  }
  return ids;
}
...
function fromTrivy(run, report, prefix, alreadyReported = new Set()) {
  for (const target of (report && report.Results) || []) {
    ...
    for (const v of target.Vulnerabilities || []) {
      // Why: the package-manager audit already reported this advisory with the fix; one result per advisory.
      if (alreadyReported.has(v.VulnerabilityID)) continue;
...
// Failing AI cases belong in the PR diff like any other finding; safety and security are errors, quality warnings.
function fromAiEval(run, report) {
  if (!report || !Array.isArray(report.cases)) return;
  const file = (report.promptFiles && report.promptFiles[0]) || CONFIG_URI;
  for (const c of report.cases) {
    if (c.status !== 'fail') continue;
    const id = `ai-eval.${c.id}`;
    run.rule(id, `AI case ${c.id} (${c.category})`);
    const level = c.category === 'safety' || c.category === 'security' ? 'error' : 'warning';
    run.add(id, level, `${c.category}: ${c.detail || 'case failing'} (measured ${report.generatedAt || 'unknown'}${report.model ? ', ' + report.model : ''})`, location(file), { category: c.category });
  }
}
...
function main() {
  ...
  const auditIds = fromAudit(run, reportOf('audit'));
  fromSemgrep(run, reportOf('semgrep'));
  fromTrivy(run, reportOf('trivy-fs'), 'trivy-fs', auditIds);
  ...
  fromAiEval(run, reportOf('ai-eval-safety'));
