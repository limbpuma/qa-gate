...
function buildLine(verdict, repo) {
  ...
  const aiSafety = byId('ai-eval-safety');
  if (aiSafety && aiSafety.report) {
    const evidence = readJson(path.join(repo, aiSafety.report));
    if (evidence && Array.isArray(evidence.cases)) {
      line.aiEval = { pass: evidence.cases.filter((c) => c.status === 'pass').length, fail: evidence.cases.filter((c) => c.status === 'fail').length };
      const quality = byId('ai-eval-quality');
      if (quality && typeof quality.value === 'number') line.aiEval.qualityPct = quality.value;
    }
  }
  return line;
}
