...
function aiEvalCard(report) {
  if (!report || !Array.isArray(report.cases)) return '';
  const rows = report.cases.map((c) => `<tr><td>${pill(c.status === 'pass' ? 'PASS' : c.status === 'fail' ? 'FAIL' : 'SKIP')}</td><td><code>${esc(c.id)}</code></td><td>${esc(c.category)}</td><td class="muted">${esc(c.detail || '')}</td></tr>`).join('');
  return `<div class="card"><h2>AI evaluation <span class="muted" style="font-weight:400">measured ${when(report.generatedAt)}${report.model ? ' · ' + esc(report.model) : ''}${report.runner ? ' · ' + esc(report.runner) : ''}</span></h2>
<table><thead><tr><th>status</th><th>case</th><th>category</th><th>detail</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
